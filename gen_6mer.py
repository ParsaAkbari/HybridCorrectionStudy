...
def process_reads(partition, before_corr_reads, after_corr_reads):
	...
	csv_obj = CsvObj("/home/pa354/c_elegans_data/analyse_nanocorr_data/6mer_csv_output/6mer_"+str(partition)+".csv")
	# loop through lines of partition csv
	
	blast_f = open("/home/pa354/c_elegans_data/analyse_nanocorr_data/make_alignment_csv_output/"+str(partition)+".csv", 'r')
       	blast_f.readline()
	# loop through each read
		...
		after_corr_spliced = after_corr_reads[line[1]][(int(line[6])-1):(int(line[7])-1)]
                ...
                before_corr_xmer_dict = make_empty_dictionary_xmer_numbers(6, {'before_corr_A' : 0, 'before_corr_T' : 0, 'before_corr_G' : 0, 'before_corr_C' : 0})
                after_corr_xmer_dict = make_empty_dictionary_xmer_numbers(6, {'after_corr_A' : 0, 'after_corr_T' : 0, 'after_corr_G' : 0, 'after_corr_C' : 0})
                before_corr_xmer_dict = make_dictionary_xmer_numbers(6, before_corr_spliced, "before_corr_", before_corr_xmer_dict)
                after_corr_xmer_dict = make_dictionary_xmer_numbers(6, after_corr_spliced, "after_corr_", after_corr_xmer_dict)
